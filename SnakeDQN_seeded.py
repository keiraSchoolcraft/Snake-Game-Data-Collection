...
class SnakeGameRLWrapper(gym.Env):
    ...
    def reset(self, seed=None):
        if seed is not None:
            ...
            random.seed(seed)
            np.random.seed(seed)
            # Also seed torch for completeness
...
class DQNAgent:
    def __init__(self, state_dim, action_dim, lr=0.0005, seed=42):
        # Set the seed for reproducibility in the agent initialization
        random.seed(seed)
        np.random.seed(seed)
        torch.manual_seed(seed)
        if torch.cuda.is_available():
    ...
    def train(self):
        ...
        # Convert to appropriate tensor shapes - fixed to use np.array first
...
def train_dqn(env, agent, num_episodes=10000, seed=42):
    """Train the agent using DQN algorithm with a fixed seed for reproducibility"""
    # Set the master seed for the entire training process
    master_seed = seed
    ...
    os.makedirs('seeded_training_logs', exist_ok=True)
    os.makedirs('best_seeded_models', exist_ok=True)
    ...
    for episode in range(num_episodes):
        # Use the same seed for each episode reset - this ensures maximum consistency
        state, _ = env.reset(seed=master_seed)
        episode_reward = 0
        episode_length = 0
        total_loss = 0
        done = False
        
        # Continue until the game naturally ends
        while not done:
            # Reset random state to ensure deterministic action selection
            # This ensures that if the agent sees the same state, it will take the same action
            # during training
            random.seed(master_seed + episode * 10000 + episode_length)
        ...
        if (episode + 1) % save_every == 0:
            ...
            checkpoint_filename = os.path.join('seeded_training_logs', f'dqn_snake_model_ep{episode+1}.pth')
            torch.save({
                ...
                'seed': seed
            }, checkpoint_filename, _use_new_zipfile_serialization=True)
            ...
            if avg_reward > best_avg_reward:
                ...
                torch.save({
                    ...
                    'epsilon': agent.epsilon,
                    'seed': seed
                }, os.path.join('best_seeded_models', 'best_reward_model.pth'), _use_new_zipfile_serialization=True)
            ...
            if avg_score > best_avg_score:
                ...
                torch.save({
                    ...
                    'epsilon': agent.epsilon,
                    'seed': seed
                }, os.path.join('best_seeded_models', 'best_score_model.pth'), _use_new_zipfile_serialization=True)
...
def play_with_trained_model(model_path='best_seeded_models/best_score_model.pth', num_games=5):
...
def main():
    ...
    if mode == 'train':
        ...
        # Set fixed seed for entire training process
        fixed_seed = 42
        
        # Create DQN agent with observation space size and the same seed
        agent = DQNAgent(
            state_dim=env.observation_space.shape[0],
            action_dim=4,
            seed=fixed_seed
        ...
        print(f"Starting training with DQN using fixed seed: {fixed_seed}...")
        rewards, lengths, losses, scores = train_dqn(env, agent, num_episodes=10000, seed=fixed_seed)
        
        # Save final training metrics
        np.savez('seeded_training_logs/final_training_metrics.npz', 
                rewards=rewards, 
                lengths=lengths, 
                losses=losses,
                scores=scores)
        ...
        plt.subplot(2, 2, 1)
        plt.plot(rewards)
        plt.title('Episode Rewards (Seeded)')
        
        plt.subplot(2, 2, 2)
        plt.plot(lengths)
        plt.title('Episode Lengths (Seeded)')
        
        plt.subplot(2, 2, 3)
        plt.plot(scores)
        plt.title('Game Scores (Seeded)')
        
        plt.subplot(2, 2, 4)
        plt.plot(losses)
        plt.title('Training Losses (Seeded)')
        
        plt.tight_layout()
        plt.savefig('seeded_training_logs/training_metrics.png')
        
        print("Seeded training completed!")
        ...
        print("Invalid mode selected. Please choose 'train' or 'play'.")
